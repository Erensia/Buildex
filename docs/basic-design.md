...
- MVP는 실전 DPS나 자동 로테이션을 추정하지 않는다. 캐릭터·무기·에코의 기초 공격력, 공격력%, 고정 공격력과 치명타·공명 효율·속성 피해 보너스를 투명하게 합산한다.
- 조건부 파티 버프는 자동 적용하지 않는다. 사용자가 발동 조건을 확인해 활성화한 버프만 결과에 반영하고, 결과 화면에 적용 조건을 함께 표시한다.
- 목표 등급은 캐릭터·무기·파티·운용 조건별 데이터 규칙으로 관리한다. 평가 결과는 달성한 최고 등급과 다음 등급에 부족한 스탯을 제공한다.
- 초기 장루브 규칙은 장리 0공명 + 전용 무기 기준의 제품 기준값이다. 실제 캐릭터·무기·에코 데이터 시드를 추가할 때 적용 패치의 공개 수치와 함께 검수한다.
...
- 홈, 로그인, 회원가입과 장리 개인 빌드 화면을 구현했다. 공개 콘텐츠 목록·상세 화면은 아직 구현하지 않았다.
- 이메일/비밀번호 로그인은 Auth.js Credentials로 구성했고, `/build` 보호 경로·세션 표시·로그아웃·로그인 후 원래 경로 복귀를 구현했다.
- 첫 지원 게임은 명조로 확정했다. 장리 0공명 + 전용 무기 빌드 입력, 조건부 파티 버프, 목표 등급 판정과 명조 3.5 검수 스냅샷 시드를 구현했다.
- 빌드 프리셋은 로그인 사용자에게만 저장되며, 서버가 계산 결과와 데이터·계산식 버전을 함께 저장한다. 현재 화면에서는 저장 목록 조회와 삭제를 제공하며, 저장값 불러오기는 후속 작업이다.
