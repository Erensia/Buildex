...
type ReleaseDiff = Record<string, { added: Array<{ name: string }>; changed: Array<{ name: string }>; removed: Array<{ name: string }> }>;
...
export function AdminDataManager({ adminName }: { adminName: string }) {
  ...
  async function loadDiff() {
    if (!release || !editable) return;
    const response = await fetch("/api/admin/game-data/releases", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "diff", releaseId: release.id }) });
    const result = await response.json();
    if (!response.ok) { setMessage(result.error ?? "변경 요약을 불러오지 못했습니다."); return; }
    setDiff(result.diff as ReleaseDiff); setMessage(`${result.baseVersion} 공개본과의 변경 요약을 불러왔습니다.`);
  }
  ...
  return <main className="neumorphic-admin min-h-screen bg-[#09070d] px-5 py-8 text-zinc-100 sm:px-8"><div className="mx-auto max-w-6xl">
    ...
    <section className="mb-6 rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5"><div className="flex flex-wrap items-end gap-3"><label className="min-w-64 flex-1 text-sm font-bold">작업 릴리스<select value={releaseId} onChange={(event) => { setReleaseId(event.target.value); setDiff(null); }} className="mt-2 w-full rounded-lg border border-zinc-700 bg-zinc-950 p-3">{releases.map((item) => <option key={item.id} value={item.id}>{item.version} · {item.status} · {item.sourceSnapshot}</option>)}</select></label><button onClick={() => void createDraft()} disabled={!game} className="rounded-lg bg-violet-500 px-4 py-3 text-sm font-bold hover:bg-violet-400 disabled:opacity-50">현재 공개본에서 초안 만들기</button>{editable && <button onClick={() => void loadDiff()} className="rounded-lg border border-violet-400/60 px-4 py-3 text-sm font-bold text-violet-200 hover:bg-violet-500/10">공개본과 비교</button>}{editable && <button onClick={() => void validateAndPublish()} className="rounded-lg border border-emerald-500/60 px-4 py-3 text-sm font-bold text-emerald-200 hover:bg-emerald-500/10">검증 후 Publish</button>}</div>{release && <p className="mt-3 text-sm text-zinc-400">{release.status === "draft" ? "초안: 저장해도 사용자에게 노출되지 않습니다." : release.status === "published" ? "현재 공개 릴리스: 직접 수정할 수 없습니다." : "보존된 과거 릴리스: 읽기 전용입니다."}</p>}{diff && <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-5">{Object.entries(diff).map(([kind, changes]) => <div key={kind} className="rounded-lg border border-zinc-800 bg-zinc-950/70 p-3 text-xs"><p className="font-bold text-zinc-200">{labels[kind as Entity] ?? kind}</p><p className="mt-2 text-emerald-300">추가 {changes.added.length}{changes.added.length ? ` · ${changes.added.map((item) => item.name).join(", ")}` : ""}</p><p className="text-amber-200">변경 {changes.changed.length}{changes.changed.length ? ` · ${changes.changed.map((item) => item.name).join(", ")}` : ""}</p><p className="text-rose-300">삭제 {changes.removed.length}{changes.removed.length ? ` · ${changes.removed.map((item) => item.name).join(", ")}` : ""}</p></div>)}</div>}</section>
