...
export function BuildPlanner({ userName }: { userName: string }) {
  ...
  function newBuild() { if (latestData) setData(latestData); setProfileId(undefined); setLoadedProfile(undefined); setName("새 빌드"); setSubStats(slots.map(() => ({}))); setActiveBuffIds([]); setPartyMemberKeys([]); setMessage("현재 공개 데이터로 새 빌드를 만듭니다."); }
