...
type EchoSet = { externalKey: string; name: string; effects: Record<string, unknown> };
...
export function BuildPlanner({ userName }: { userName: string }) {
  ...
  function payload(): BuildInput {
    return { name, characterKey, weaponKey: weaponKey || weapon?.externalKey || "", echoes: slots.map((slot, index) => ({ slot: slot.slot, echoKey: echoKeys[index], setKey: setKeys[index] ?? "", cost: slot.cost, mainStat: mainStats[index], subStats: index === 0 ? Object.entries(subStats).map(([key, value]) => ({ key, value: value ?? 0 })) : [] })), activeBuffIds, formulaVersion: FORMULA_VERSION };
  }
  async function save() {
  ...
  function load(profile: SavedProfile) {
    ...
    setProfileId(profile.id); setName(input.name); setCharacterKey(input.characterKey); setWeaponKey(input.weaponKey); setSetKeys(input.echoes.map((echo) => echo.setKey)); setEchoKeys(input.echoes.map((echo) => echo.echoKey ?? "")); setMainStats(input.echoes.map((echo) => echo.mainStat)); setSubStats(Object.fromEntries(input.echoes.flatMap((echo) => echo.subStats.map((stat) => [stat.key, stat.value] as const)))); setActiveBuffIds(input.activeBuffIds); setMessage(`Loaded ${profile.name}.`);
  }
  ...
  return <main className="min-h-screen bg-zinc-950 pb-16 text-zinc-100">
    ...
    <div className="mx-auto grid max-w-6xl gap-6 px-6 lg:grid-cols-[1.2fr_.8fr]"><section className="space-y-6">
      ...
      <section className="rounded-3xl border border-zinc-800 bg-zinc-900/60 p-6"><h2 className="font-bold">Echo setup</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{slots.map((slot, index) => <div key={slot.slot} className="rounded-xl bg-zinc-950/50 p-3"><label className="block">Echo {slot.slot} · Cost {slot.cost}<select className="mt-2 w-full rounded-xl bg-zinc-950 p-3" value={echoKeys[index] ?? ""} onChange={(e) => setEchoKeys((items) => items.map((item, i) => i === index ? e.target.value : item))}>{data.echoes.filter((item) => item.cost === slot.cost).map((item) => <option key={item.externalKey} value={item.externalKey}>{item.name}</option>)}</select></label><label className="mt-3 block">Echo set<select className="mt-2 w-full rounded-xl bg-zinc-950 p-3" value={setKeys[index] ?? ""} onChange={(e) => setSetKeys((items) => items.map((item, i) => i === index ? e.target.value : item))}>{data.echoSets.map((item) => <option key={item.externalKey} value={item.externalKey}>{item.name}</option>)}</select></label><label className="mt-3 block">Main stat<select className="mt-2 w-full rounded-xl bg-zinc-950 p-3" value={mainStats[index] ?? ""} onChange={(e) => setMainStats((items) => items.map((item, i) => i === index ? e.target.value : item))}>{optionsFor(slot.cost).map((item) => <option key={item.statKey} value={item.statKey}>{statLabels[item.statKey]} +{item.value}%</option>)}</select></label></div>)}</div></section>
