...
export function BuildPlanner({ userName }: { userName: string }) {
  ...
  return <main className="min-h-screen bg-zinc-950 pb-16 text-zinc-100">
    ...
    <div className="mx-auto grid max-w-6xl gap-6 px-6 lg:grid-cols-[1.2fr_.8fr]"><section className="space-y-6">
      <div><p className="text-sm font-semibold tracking-widest text-violet-400">BUILD PLANNER</p><h1 className="mt-2 text-4xl font-black">빌드 구성</h1>{data.games[0] && <p className="mt-2 text-xs text-zinc-500">{data.games[0].name} 데이터 v{data.games[0].currentDataVersion ?? "알 수 없음"} · 검증일 {data.games[0].sourceSnapshot ?? "알 수 없음"}{data.games[0].sourceUrl && <> · <a className="underline hover:text-zinc-300" href={data.games[0].sourceUrl} rel="noreferrer" target="_blank">출처</a></>}</p>}</div>
      <section className="rounded-3xl border border-zinc-800 bg-zinc-900/60 p-6"><h2 className="font-bold">캐릭터와 무기</h2><div className="mt-4 grid gap-3 sm:grid-cols-2"><label>캐릭터<select className="mt-2 w-full rounded-xl bg-zinc-950 p-3" value={characterKey} onChange={(e) => { setCharacterKey(e.target.value); setWeaponKey(""); }}><option value="" disabled>캐릭터를 선택하세요</option>{data.characters.map((item) => <option key={item.externalKey} value={item.externalKey}>{item.name}</option>)}</select></label><label>무기<select className="mt-2 w-full rounded-xl bg-zinc-950 p-3" value={weapon?.externalKey ?? ""} onChange={(e) => setWeaponKey(e.target.value)}>{compatibleWeapons.map((item) => <option key={item.externalKey} value={item.externalKey}>{item.name}</option>)}</select></label></div></section>
    ...
    <aside className="h-fit rounded-3xl border border-violet-500/30 bg-zinc-900 p-6 lg:sticky lg:top-6"><p className="text-sm text-violet-300">CALCULATED RESULT</p>{calculation && <><p className="mt-3 text-3xl font-black">ATK {format(calculation.result.attack)}</p><dl className="mt-4 grid grid-cols-2 gap-3 text-sm">{([["CRIT Rate", calculation.result.critRate], ["CRIT DMG", calculation.result.critDamage], ["Energy Regen", calculation.result.energyRegen], ["Fusion DMG", calculation.result.fusionDamageBonus]] as const).map(([label, value]) => <div className="rounded-xl bg-zinc-950 p-3" key={label}><dt className="text-zinc-500">{label}</dt><dd className="font-bold">{format(value)}%</dd></div>)}</dl>{calculation.grade && <><p className="mt-4 text-sm text-zinc-300">Grade: {calculation.grade.grade ?? "Not graded"}</p>{calculation.grade.unmetRequirements.length > 0 && <div className="mt-4 rounded-xl border border-violet-500/30 bg-violet-950/20 p-3 text-sm"><p className="font-semibold text-violet-200">Next grade recommendations</p><ul className="mt-2 space-y-1 text-zinc-300">{calculation.grade.unmetRequirements.map((requirement) => <li key={requirement.stat}>{requirement.label}: +{format(requirement.minimum - calculation.result[requirement.stat])}</li>)}</ul></div>}</>}</>}
      ...
      <div className="mt-6 border-t border-zinc-700 pt-5"><input aria-label="빌드 이름" className="w-full rounded-xl bg-zinc-950 p-3" value={name} maxLength={80} onChange={(e) => setName(e.target.value)} /><div className="mt-3 flex gap-3"><button className="rounded-xl bg-violet-500 px-4 py-2 font-bold disabled:opacity-50" disabled={saving || !name.trim()} onClick={save}>{saving ? "저장 중…" : profileId ? "빌드 수정" : "빌드 저장"}</button><button className="rounded-xl border border-zinc-700 px-4 py-2" onClick={newBuild}>새 빌드</button></div>{message && <p className="mt-3 text-sm text-zinc-300">{message}</p>}</div>
      ...
      <div className="mt-6 border-t border-zinc-700 pt-5"><h2 className="font-bold">저장된 빌드</h2>{profiles.length === 0 ? <p className="mt-3 text-sm text-zinc-500">저장된 빌드가 없습니다.</p> : <ul className="mt-3 space-y-3">{profiles.map((profile) => <li className="flex items-center justify-between gap-2" key={profile.id}><button className="min-w-0 truncate text-left text-sm font-semibold" onClick={() => load(profile)}>{profile.name}</button><div className="flex gap-2 text-xs"><button className="text-zinc-300" onClick={() => duplicate(profile)}>복제</button><button className="text-rose-300" onClick={() => { if (comparisonProfileId === profile.id) setComparisonProfileId(undefined); void remove(profile.id); }}>삭제</button></div></li>)}</ul>}</div>
    </aside></div>
  </main>;
}
